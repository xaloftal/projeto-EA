...
public class CheckOutTransportService {
    ...
    public CheckOutResponseDTO checkOut(CheckOutRequestDTO request) {
        ...
        Title title = titleRepository.findById(request.titleId())
                .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND,
                        "Title not found: " + request.titleId()));

        Trip trip = tripRepository.findById(request.tripId())
                .orElseThrow(
                        () -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Trip not found: " + request.tripId()));
    ...
    private CheckOutResponseDTO checkOutTicket(Ticket ticket, Trip trip) {
        ...
        if (isAtDestination) {
            message = "You are at the destination stop.";
            isCorrectExit = true;
        } else {
            message = "You haven't arrived at the destination stop yet.";
            isCorrectExit = false;
        }
